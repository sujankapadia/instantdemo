...
| EL4 | Phase-4 state cost with a fake dispatcher whose session_cost_totals grows under ONE SDK-UUID key (never our logical phase4-…-cN ids); EL1/EL2 keep dispatcher=None | state.json phase-4 cost_usd == the dispatcher-total growth across the loop (per-section before/after snapshots summed) — key-agnostic | The live $0.00 bug: looking up by logical session id misses (the SDK keys results by its own UUID), so the GUI costs panel shows $0 for the most expensive phase |
| EL5 | A chapter's findings keyed by LOCAL 1-based positions instead of the global Segment numbers; corrected payload returned on the re-ask | ONE corrective re-ask in the SAME session (JSON-only, no browser re-run); corrected findings merge; the chapter's scenes end verified | The live revise bug: the scope guard silently ignored every PASS finding while the section concluded OK — record armed, phase 5 rejected the board |
| EL6 | Findings indices still out of scope after the corrective re-ask | The section refuses OK (in-scope scenes still hypothesized → BLOCKED); the run raises at rehearsal time, not at phase 5 | A lying gate: "Looks good — record it" over an unverified chapter |
...
(GL1/GL2 share one test — the canned harness asserts in-loop validation; EL3 is covered by the existing M5b suite (test_scoped_replan) remaining green plus the live rehearsal smoke. EL4 reuses the EL harness with a fake dispatcher injected; EL1/EL2 run with dispatcher=None as before. EL5/EL6 reuse the harness with a `local_indices_chapter` whose first payload renumbers from 1; the corrective re-ask is identified by its "Re-emit ONLY the corrected" prompt text and either fixes the indices (EL5, `correction_fixes=True`) or repeats the mistake (EL6, `correction_fixes=False`).)
