...
| **One-object pass** ✅ 2026-06-11 | Design consolidation (DESIGN.md) | ONE maturing Stage (front door → filmstrip → proposal → storyboard → film), brief-box intake, Inspector replaces the mode toggle, film vocabulary, screening-room tokens + motion + waveforms | DESIGN.md principles 1–17; frontend-only (backend diff empty) | L |
| **M4** (rescoped 2026-06-10) ✅ 2026-06-12 | Revision essentials | **Versioned takes** (snapshot per render/revision, A/B previous-version toggle, restore) + a whole-demo "adjust style/pacing" pass (one text box → one rewrite/pace change → one audio re-render; no classifier) | Takes design + SDK-no-tools pattern from the M4 design review; segment edit/delete/re-voice already cover per-segment revision directly | M |
| **M5a** ✅ 2026-06-12 | Chapters exist | scene.section + chapters() in the storyboard contract; Phase 2 plans in 2-6 named chapters (contiguity validated); chapter headers w/ status roll-up on the storyboard; **gated Regenerate** (#82 stage 1 — staged flow w/ both gates; pre-render take guarantees the old film survives) | M0 contract extension + the M2 gate flow | M |
| **M5b** ✅ 2026-06-12 | Scoped operations (#50) → **the feedback milestone** (M5a L5 findings, settled: the "Revise this chapter" affordance lives on the FILM FACE's scenes pane — post-render that's where the user is — with the storyboard keeping its headers for the gate; and during re-planning the stale storyboard must read as outgoing — dim + "previous plan — a new one is being written" badge) | Section schema in storyboard/script, per-section re-plan + re-record, chapter UX; structural notes ("redo the middle", "also show X") become affordable scoped operations | The §2.1 analysis (PRODUCT_DIRECTION.md); render concat is straightforward ffmpeg; the M4 design review's answers-card voice + regenerate-with-brief door carry over | L |
| **M6** ✅ 2026-06-12 | Output package v1 | SRT captions (display text — M3's split; frame-true since the M5b timing fixes), one-click film+captions zip download. Logo watermark and outro card were built, then CUT in the L5 design review (a watermark covers the UI the demo exists to show; the outro's value didn't justify plumbing through four render paths) | All ffmpeg-layer, no agent work | S |
| **M7** ✅ 2026-06-13 | The chaptered cold start | Outline → per-chapter plan/inspect/rehearse (single-session continuations for 2/3, isolated sessions for 4); continuity pass; chapter progress in the header; legacy boards degrade gracefully. Lifts the ~25-scene length ceiling: prompts O(chapter), cost linear in length | The M5b scoped machinery generalized; the live gate caught and fixed a 10x session-economics bug | L |
