...
def resolve_and_act(page, step):
    ...
    if step.action == "fill":
        ...
        loc.fill(step.value or 'Demo item'); loc.press('Enter'); return
    ...
    with sync_playwright() as pw:
        ...
        for i, path in enumerate(plan.paths, start=2):
            try:
                ...
                snap = snapshot(page, i, path.reaches)
                snap["elems"] = [x["text"] for x in interactive_dump(page)]
                screens.append({**snap, "reaches": path.reaches,
...
REPLAN_TPL = """You already explored a web app from its home page. Doing certain actions
revealed NEW controls that were NOT visible on the home page:

{discoveries}

These often gate a whole screen (e.g. an empty list's filters appear only after you add an
item). Plan up to 4 `paths` to explore the most demo-relevant NEW screens. CRITICAL: each
path must first REPLAY the action that revealed the control, THEN interact with the new
control. Use EXACT visible text. setup = []."""


async def replan_call(discoveries):
    be = PydanticAIBackend(default_model="openrouter:deepseek/deepseek-chat-v3.1",
                           allowed_roots=[P("/tmp")], cwd=P("/tmp"))
    class C: event_emitter = None
    t = time.monotonic()
    r = await be.run_structured(C(), REPLAN_TPL.format(discoveries=discoveries),
                                "phase2-replan", output_type=ExplorePlan)
    print(f"REPLAN ({time.monotonic()-t:.0f}s): {len(r.output.paths)} round-2 paths")
    for pth in r.output.paths: print(f"  path → {pth.reaches}: " + " → ".join(f"{s.action} {s.target!r}" for s in pth.steps))
    return r.output
...
with sync_playwright() as _pw:
    ...
    home_elems = [x['text'] for x in interactive_dump(_pg)]
    HOME_SET = set(home_elems)
...
print(f"\nEXEC round1 ({time.monotonic()-te:.0f}s): {len(screens)} screens, {failures} failures")
for s in screens:
    print("  ", ("FAIL " if s.get("error") else "ok   "), s.get("reaches"), "|", s.get("error") or (s.get("did") or "")[:70])

# ROUND 2 — discover controls revealed by round-1 actions, explore them
disc_lines = []
for s in screens:
    if s.get("error") or not s.get("elems"): continue
    new = [e for e in dict.fromkeys(s["elems"]) if e and e not in HOME_SET][:8]
    if new and s.get("did"):
        disc_lines.append(f'After "{s["did"]}": new controls → ' + ", ".join(repr(e) for e in new))
if disc_lines:
    print(f"\nDISCOVERED new controls behind {len(disc_lines)} action(s) — running round 2")
    plan2 = asyncio.run(replan_call("\n".join(disc_lines[:8])))
    te2 = time.monotonic()
    screens2, f2 = run_plan(plan2)
    print(f"EXEC round2 ({time.monotonic()-te2:.0f}s): {len(screens2)-1} new screens, {f2} failures")
    for s in screens2[1:]:
        print("  ", ("FAIL " if s.get("error") else "ok   "), s.get("reaches"), "|", s.get("error") or (s.get("did") or "")[:70])
    screens += screens2[1:]   # drop round-2's duplicate baseline
else:
    print("\nNo new gated controls discovered — single round sufficed")
